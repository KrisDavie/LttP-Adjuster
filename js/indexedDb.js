...
IndexedDb.prototype.setFormValues = function () {
  ...
  el("checkbox-renamemulti").checked = this.obj.rename_multi;
  el("input-renamemulti").value = this.obj.multi_names;
  var multiDivs = document.getElementsByClassName("renamemulti");
  if (this.obj.rename_multi) {
    for (var i = 0; i < multiDivs.length; i++) {
      var div = multiDivs[i];
  ...
  el("checkbox-renamemulti2").checked = this.obj.rename_multi;
  el("input-renamemulti2").value = this.obj.multi_names;
  var multiDivs = document.getElementsByClassName("renamemulti2");
  if (this.obj.rename_multi) {
    for (var i = 0; i < multiDivs.length; i++) {
      var div = multiDivs[i];
      div.style.display = "block";
...
IndexedDb.prototype.save = function (tab) {
  ...
  this.obj.rename_multi = el("checkbox-renamemulti" + id).checked;
  this.obj.multi_names = el("input-renamemulti" + id).value;
